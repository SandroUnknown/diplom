...
> | `WEB_REMOTE_URL`      | Обязательный для <br> ` remote-web ` тестов     | Адрес удалённого сервера (для запуска web-тестов)              | ??????????                       |
...
> | `BROWSERSTACK_URL`    | Обязательный для <br> ` remote-android ` тестов | Адрес `browserstack` (для удалённого запуска android-тестов)   | ?????????????                    |

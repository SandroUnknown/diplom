...
> | Параметр          | Описание                                                     | Значение по умолчанию                |
> | :---              | :---                                                         | :---                                 |
> | `TASK`            | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | _`test`_                                 |
> | **`REMOTE_URL`** | Адрес удалённого сервера, где будут запускаться тесты <br><br> **Обязательный параметр при запуске WEB-тестов** | Нет значения по умолчанию |
> | **`REMOTE_URL`** <br><br> Обязательный | Адрес удалённого сервера, где будут запускаться тесты <br><br> **Обязательный параметр при запуске WEB-тестов** | Нет значения по умолчанию |
> | **`REMOTE_URL` <br><br> (Обязательный)** | Адрес удалённого сервера, где будут запускаться тесты <br><br> **Обязательный параметр при запуске WEB-тестов** | Нет значения по умолчанию |
> | **`REMOTE_URL`** <br> _(Обязательный)_ | Адрес удалённого сервера, где будут запускаться тесты <br><br> **Обязательный параметр при запуске WEB-тестов** | Нет значения по умолчанию |
> | **`REMOTE_URL` <br> (Обязательный)** | Адрес удалённого сервера, где будут запускаться тесты <br><br> **Обязательный параметр при запуске WEB-тестов** | Нет значения по умолчанию |
> | `BASE_URL`        | Адрес сайта, где будут запускаться тесты                     | https://flat-soft.ru                 |
> | `BROWSER`         | Браузер, в котором будут выполняться тесты                   | chrome                               |
> | `BROWSER_VERSION` | Версия браузера, в которой будут выполняться тесты           | 125.0                                |
> | `BROWSER_SIZE`    | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080                            |
...
> | :---              | :---          | :---                                                         | :---                                 |
> | `TASK`            | Обязательный  | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | test                                 |
> | `REMOTE_URL`      | Обязательный* | Адрес удалённого сервера, где будут запускаться тесты        | (пустое значение ~ локальный запуск) |

...
> | :---              | :---                                                         | :---                                   | :---                  |
> | `TASK`            | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | **Обязательный всегда**                    | (нет)                 |
> | `REMOTE_URL`      | Адрес удалённого сервера, где будут запускаться тесты        | Обязательный при запуске web-теста     | (нет)                 |
> | `BASE_URL`        | Адрес сайта, где будут запускаться тесты                     | Обязательный при запуске android-теста | (нет)                 |

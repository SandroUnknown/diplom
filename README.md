...
> | `TASK`                | Обязательный для `всех` тестов | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | test                  |
...
> | `WEB_URL`             | Обязательный | Адрес сайта, где будут запускаться тесты                     | https://flat-soft.ru  |
...
> | `WEB_REMOTE_URL`      | Обязательный | Браузер, в котором будут выполняться тесты                   | chrome                |
> | `WEB_REMOTE_LOGIN`    | Обязательный | Версия браузера, в которой будут выполняться тесты           | 125.0                 |
> | `WEB_REMOTE_PASSWORD` | Обязательный | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
> | `BROWSERSTACK_URL`    | Обязательный | Браузер, в котором будут выполняться тесты                   | chrome                |
> | `BROWSERSTACK_USER`   | Обязательный | Версия браузера, в которой будут выполняться тесты           | 125.0                 |
> | `BROWSERSTACK_KEY`    | Обязательный | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
>

...
> | :---              | :---                                                         | :---                                     | :---                  |
> | `TASK`            | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | **Обязательный всегда**                  | test                  |
> | `REMOTE_URL`      | Адрес удалённого сервера, где будут запускаться тесты        | Обязательный при запуске `web-теста`     | _(нет)_               |
> | `BASE_URL`        | Адрес сайта, где будут запускаться тесты                     | Обязательный при запуске `android-теста` | _(нет)_               |
> | `BROWSER`         | Браузер, в котором будут выполняться тесты                   | _Необязательный_                         | 111                   |
> | `BROWSER_VERSION` | Версия браузера, в которой будут выполняться тесты           | _Необязательный_                         | 22222                 |
> | `BROWSER_SIZE`    | Размер окна браузера, в котором будут выполняться тесты      | _Необязательный_                         | 3333333               |

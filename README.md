...
> | `TASK`                | Обязательный <br> _(требуется для `всех` тестов)_ | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | test                  |
...
> | `API_URI`             | Обязательный <br> `(требуется для всех тестов)` | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
> | `WEB_URL`             | Опциональный | Адрес сайта, где будут запускаться тесты                     | https://flat-soft.ru  |
> | `TODOIST_EMAIL`       | Обязательный <br> `(требуется для всех тестов)` | Браузер, в котором будут выполняться тесты                   | chrome                |
> | `TODOIST_PASSWORD`    | Обязательный <br> `(требуется для всех тестов)` | Версия браузера, в которой будут выполняться тесты           | 125.0                 |
> | `TODOIST_TOKEN`       | Обязательный <br> `(требуется для всех тестов)` | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
> | `WEB_REMOTE_URL`      | Опциональный | Браузер, в котором будут выполняться тесты                   | chrome                |
> | `WEB_REMOTE_LOGIN`    | Опциональный | Версия браузера, в которой будут выполняться тесты           | 125.0                 |
> | `WEB_REMOTE_PASSWORD` | Опциональный | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
> | `BROWSERSTACK_URL`    | Опциональный | Браузер, в котором будут выполняться тесты                   | chrome                |
> | `BROWSERSTACK_USER`   | Опциональный | Версия браузера, в которой будут выполняться тесты           | 125.0                 |
> | `BROWSERSTACK_KEY`    | Опциональный | Размер окна браузера, в котором будут выполняться тесты      | 1920x1080             |
>

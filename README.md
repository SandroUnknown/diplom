...
> Для корректного запуска тестов обязательно должны быть переданы параметры, отмеченные как `Обязательный`, но не имеющие значения по умолчанию.</br>
...
> | `ENV`                 | Обязательный для <br> `всех` тестов             | Окружение запуска </br> _(доступные варианты: </br> -`local` </br> -`remote`)_ | local                                                        |
...
> | `WEB_REMOTE_URL`      | Обязательный для <br> ` remote-web ` тестов     | Адрес удалённого сервера (для запуска web-тестов)                        | [selenoid.autotests.cloud](https://selenoid.autotests.cloud) |
...
> ```
> gradle clean test -Dtodoist_email="sample@email.com" -Dtodoist_password="pass123" -Dtodoist_token="qW12Rty"
> ```

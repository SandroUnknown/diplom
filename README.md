...
> | Параметр          | Описание                                                     | Обязательный для теста <br> API WEB ANDROID        |
> | :---              | :---                                                         | :---                                 |
> | `TASK`            | Имя задачи в Gradle</br> _(указывает какие тесты выполнить)_ | [x] [x] [x]                                |
> | `REMOTE_URL`      | Адрес удалённого сервера, где будут запускаться тесты        | [x] [ ] [x] |
> | `BASE_URL`        | Адрес сайта, где будут запускаться тесты                     |                 |
